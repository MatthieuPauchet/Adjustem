...
<div class="row">
    <div class="col-12">
        <a href="<?= site_url("Welcome/initialisation") ?>" class="btn <?= $config->con_bouton_continuer?>">Ajouter une menuiserie</a>
        <?php if (count($panier) != 0) { ?>
            <?php if ($this->auth->is_logged()) : ?>
                <a href="<?= site_url('PanierValidation') ?>" class="btn <?= $config->con_bouton_valider?>">Valider mon devis</a>
                <a href="<?= site_url('PanierValidation/enregistrer') ?>" class="btn <?= $config->con_bouton_valider?>">Enregistrer mon panier</a>
            <?php else : ?>
                <span>Veuillez vous connecter pour valider votre devis</span>
            <?php endif; ?>
            <a href="<?= site_url("Panier/Clean") ?>" class="btn <?= $config->con_bouton_annuler?>">Vider Panier</a>
        <?php } ?>
    </div>
</div>