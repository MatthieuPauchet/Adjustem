...
CREATE TABLE menuiserie(
   ...
   men_nombre_vantaux INT NOT NULL DEFAULT 1,
...
insert into detail (det_id,det_nom,det_descriptif,det_prix,det_coefficient,det_photo,det_rub_id) values (1,'Pose en neuf','Cas de construction neuve',0,1,'detail1.svg',4);
insert into detail (det_id,det_nom,det_descriptif,det_prix,det_coefficient,det_photo,det_rub_id) values (2,'Pose en réhabilitation totale','Dépose complète de la menuiserie existante compris bâti',0,1.2,'detail2.svg',4);
insert into detail (det_id,det_nom,det_descriptif,det_prix,det_coefficient,det_photo,det_rub_id) values (3,'Pose en réhabilitation partielle','Dépose uniquement des ouvrants, conservation du bâti',0,0.8,'detail3.svg',4);
...
INSERT INTO employe (emp_nom,emp_prenom,emp_mail,emp_password) VALUES ('toto','toto','toto@mail.fr','toto');
...
CREATE TABLE config(
   con_id INT NOT NULL,
   con_police VARCHAR(50),
   con_logo VARCHAR(250),
   con_bouton_continuer VARCHAR(50),
   con_bouton_retour VARCHAR(50),
   con_bouton_valider VARCHAR(50),
   con_bouton_annuler VARCHAR(50),
   con_emp_id INT,
   PRIMARY KEY(con_id),
   FOREIGN KEY(con_emp_id) REFERENCES employe(emp_id)
);

INSERT INTO config (con_id, con_police, con_logo, con_bouton_continuer, con_bouton_retour, con_bouton_valider, con_bouton_annuler, con_emp_id) VALUES (1,'Arial','logo_adjustem.png','btn-info','btn-secondary','btn-success','btn-danger',1);
