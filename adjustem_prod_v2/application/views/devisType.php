...
<form action="<?= site_url('DevisType/validation') ?>" method="post">
        ...
        foreach ($details as $detail) {
            ...
            <div class="col-12 col-sm-6 col-md-4 col-xl-3">
                <div class="card MLD20" style="width: 18rem">
                    <div class="card-header text-center headerCard_devisType">
                        <h5 class="card-title"><?= $detail->det_nom ?></h5>
                    </div>
                    <div class="card-body bodyCard_devisType">
                        <label>
                            <input class="form-check-input" type="radio" name="det_id" value="<?= $detail->det_id ?>" required <?php $count == 1 ? print "checked" : "" ?>>
                            <img class="card-img-top img_devisType" src="<?= base_url("assets/images/") . $detail->det_photo ?>" alt="<?= $detail->det_photo ?>">
                        </label>
                    </div>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-4 col-xs-3 col-md-2 col-xl-1">
            <button type="submit" id="submit_type" class="btn <?= $config->con_bouton_continuer?>" disabled>continuer</button>
        </div>
        <div class="col-8 col-xs-6">
            <a class="btn <?= $config->con_bouton_retour?>" href="<?= site_url("DevisPose/annulation") ?>">Retour sur la pose</a>
        </div>
    </div>
</form>